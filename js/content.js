...
$body.append("<style>" +
    ...
    "</style>" +
    "<div id='z_menu' hidden style='position: fixed; z-index:99999; width: 250px; font-size: 12px; text-align: left; background: rgba(255, 255, 255, 1); border: 1px solid #bababa; box-shadow: 1px 1px 3px #888888;'>" +
...
$(function () {
  // 与后台建立连接
  var port = chrome.runtime.connect({name: "content_background"});
  ...
  // region 初始化
  // 初始化主体内容高为可视区域高度，受浏览器缩放影响
  mainDiv.height(document.documentElement.clientHeight);
  // 初始化列表高度（包括内边距和边框） = 主体内容高度
  tabsDiv.outerHeight(mainDiv.height());
  // 初始化列表宽度
  setWidth();
  // 默认隐藏
  ...
  function setWidth(mainDivWidth) {
    ...
    // 改变列表宽度（包括内边距和边框） = 主体内容宽度 - 边框宽度 - 显示块宽度
    tabsDiv.outerWidth(mainDiv.innerWidth() - borderDiv.width() - showDiv.width());
  }
