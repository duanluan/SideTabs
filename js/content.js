...
$(function () {
    ...
    function showAndHideTabAction(isShow, iconWidth) {
        var $this = $(this);
        ...
        // 音量图标不隐藏
        var paddingRight = 0;
        if ($this.find(".icon-audible").length == 1 || $this.find(".icon-muted").length == 1) {
            iconSize -= 1;
    ...
    borderDiv.mousedown(function () {
        ...
        // 拖动边缘时宽度随鼠标的左右移动而改变
        doc.mousemove(function (event) {
            // event.clientX 为鼠标指针相对于浏览器页面的水平坐标
    ...
    // 是否已经添加了 onHighlighted 监听，防止多次添加
    var addedHighlightedListener = false;
    // 发送消息，初始化标签页列表
    port.postMessage({requestType: RequestTypeEnum.GET_ALL_TABS, addedHighlightedListener: addedHighlightedListener});
    // 监听消息，与 background 通信
    port.onMessage.addListener(function (msg) {
        var requestType = msg.requestType;

        if (requestType == RequestTypeEnum.GET_ALL_TABS) {
            tabsDiv.empty();
            $.each(msg.tabs, function (i, tab) {
                ...
                var iconWidth = 14;
                tabsDiv.append("<li style='position: relative; padding: 5px 0; padding-left: 23px; border-bottom: 1px solid #ccc; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; background: url(" + iconUrl + ") no-repeat 0 50% / 18px'>" +
                ...
                // 标签页是否有声音
                if (tab.audible) {
                    tabActionDiv.prepend("<div class='iconfont icon-audible' style='position: absolute; right: " + iconSize * iconWidth + "px; margin-top: -" + iconWidth / 2 + "px; height: " + iconWidth + "px; width: " + iconWidth + "px; line-height: " + iconWidth + "px; text-align: center; font-size: 8px; border-radius: 50%'></div>");
                    tabLi.css("padding-right", tabLi.find(".icon-pinned").width() + iconWidth);
                    tabLi.find(".icon-audible").mouseover(function () {
                        $(this).attr("class", "iconfont icon-muted");
                    });
                    tabLi.find(".icon-audible").mouseleave(function () {
                        $(this).attr("class", "iconfont icon-audible");
                    });
                }
                // 标签页是否静音
            ...
            // 首次获取标签页时就改变状态，保证监听只被添加一次
            if (!addedHighlightedListener) {
                addedHighlightedListener = true;
            }
        }
        /*else if(requestType == RequestTypeEnum.GOTO_TAB) {
                }*/
    });
    ...
    // 静音和取消静音标签页
    $body.on("click", ".icon-audible", function () {
        var $this = $(this);
        ...
        var tabId = parseInt($(this).parent().parent().find(".z_tab_id").text());
        port.postMessage({requestType: RequestTypeEnum.AUDIBLE_TAB, tabId: tabId});
